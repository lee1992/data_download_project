...
def plot_pivots(df: pd.DataFrame,
                ...
                figsize: Tuple = (16, 6),
                save_path: str = 'pivots_plot.png'):  # 新增：保存路径
    """
    绘制 K 线 + ZigZag 折线 + pivot 标注，直接保存为图片文件。
    """
    try:
...
def plot_multi_scale(df: pd.DataFrame,
                     ...
                     figsize: Tuple = (18, 10),
                     save_path: str = 'multi_scale_plot.png'):  # 新增：保存路径
    """
    绘制多尺度 pivot 对比图，直接保存为图片文件。
    """
    try:
        import matplotlib.pyplot as plt
        import matplotlib.dates as mdates
...
def demo_phase_0_1():
    ...
    try:
        plot_pivots(df, pivots_with_anchors,
                    ...
                    save_path='qqq_pivots_20260426.png')  # 指定保存文件名
